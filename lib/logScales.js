...
/** Payment processing fee: linear 0%–10% of monthly revenue; slider still uses 0–1000 internally. */
const BANK_PCT = [0, 10];
...
/** 0% cash … 100% card; slider 0–1000 → 0.0–100.0% in 0.1 steps. */
const CARD_PCT = [0, 100];
export function cardShareToSlider(pct) {
  const [mn, mx] = CARD_PCT;
  const p = Math.max(mn, Math.min(mx, Number(pct) || 0));
  return Math.round(((p - mn) / (mx - mn)) * 1000);
}

export function cardShareFromSlider(t) {
  const [mn, mx] = CARD_PCT;
  t = Math.max(0, Math.min(1000, +t));
  return Math.round((mn + (t / 1000) * (mx - mn)) * 10) / 10;
}
...
export function fromSlider(scale, t) {
  ...
  if (t === 0) return mn;
  if (t === 1000) return mx;
  const raw = mn * Math.pow(mx / mn, t / 1000);
  if (scale === "price") {
