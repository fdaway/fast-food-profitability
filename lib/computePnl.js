...
/**
 * @param {object} input
 * @param {number} input.price  Selling price per item (incl. any combo add-ons the caller adds)
...
export function computePnl({
  ...
  /** Fee % (op.monthly) applies only to the card share of revenue (op.cardPct, default 50). */
  const bankFeeOnRev = (op) =>
    (Number(op.monthly) / 100) * (Number(op.cardPct ?? 50) / 100);

  const monthOps = opsItems.reduce(
    (s, op) =>
      op.isBank
        ? s + monthRev * bankFeeOnRev(op)
        : s + op.monthly,
    0
  );
  ...
  const unitsPerMonth = items * days;
  const profitPerItem =
    unitsPerMonth > 0 ? monthProf / unitsPerMonth : null;

  const fixedM =
    monthLabor +
    monthRent +
    opsItems.reduce((s, op) => (op.isBank ? s : s + op.monthly), 0);
  const bankRate = opsItems
    .filter((o) => o.isBank)
    .reduce((s, o) => s + bankFeeOnRev(o), 0);
